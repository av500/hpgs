/***********************************************************************
 *                                                                     *
 * $Id: hpgspen.c 531 2010-05-25 19:47:39Z softadm $
 *                                                                     *
...
int hpgs_device_setrgb_all(hpgs_device *dev, const hpgs_color *rgb) {

  if (hpgs_setrgbcolor(dev,rgb))
    return -1;

  return hpgs_setpatcol(dev,rgb);
}
...
int hpgs_reader_do_PC (hpgs_reader *reader)
{
  ...
  if (pen == reader->current_pen)
    if (hpgs_device_setrgb_all(reader->device,
                               &reader->pen_colors[pen]))
      return -1;
...
int hpgs_reader_do_LA(hpgs_reader *reader)
{
  ...
  while (!reader->eoc)
	...
	case 3:
	  ...
	  if (rvalue <= 32768.0 && hpgs_setmiterlimit(reader->device,rvalue)) return -1;
	  break;
	case 1:
