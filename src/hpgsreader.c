/***********************************************************************
 *                                                                     *
 * $Id: hpgsreader.c 496 2009-03-11 08:43:54Z softadm $
 *                                                                     *
                             ...
                             hpgs_bool ignore_lt )
{
  ...
  ret->pcl_n_palettes = 32;

  ret->pcl_palettes = (hpgs_reader_pcl_palette**)
    malloc(sizeof(hpgs_reader_pcl_palette *)*ret->pcl_n_palettes);
      ...
      if (reader->verbosity >= 2)
        {
          ...
          hpgs_log("Command %c%c found at file position %lu.\n",
                       ...
                       double stamp_size)
{
  ...
  if (hpgs_device_setrgb_all(dev,&rgb))
    return -1;
...
void hpgs_destroy_reader(hpgs_reader *reader)
{
  ...
  if (reader->pcl_palettes)
    free(reader->pcl_palettes);
